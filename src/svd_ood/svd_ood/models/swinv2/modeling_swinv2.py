...
"""Add local image feature to Swinv2Model. https://arxiv.org/pdf/2304.04521.pdf"""
...
class Swinv2ImageClassifierOutput(ModelOutput):
    ...
    Args:
        ...
        reshaped_hidden_states (`tuple(torch.FloatTensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            ...
            Hidden-states of the model at the output of each layer plus the initial embedding outputs reshaped to
            include the spatial dimensions.
        logits_local (`tuple(torch.FloatTensor)`, *optional*, returned when `compute_local_feat=True` is set by `model.enable_local_feat`:
            Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, sequence_length, config.num_labels)`.
    """
    ...
    reshaped_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    # >>> lyx: Add for local feature >>>
    logits_local: Optional[Tuple[torch.FloatTensor]] = None
    # <<< End <<<
...
class Swinv2Layer(nn.Module):
    def __init__(self, config, dim, input_resolution, num_heads, shift_size=0, pretrained_window_size=0):
        ...
        self.intermediate = Swinv2Intermediate(config, dim) # fc1
        self.output = Swinv2Output(config, dim)             # fc2
    ...
    def forward(
        ...
        def residual_attn(hidden_states):
            ...
            layer_output = self.intermediate(hidden_states) # fc1
            layer_output = self.output(layer_output)        # fc2
        ...
        def local_feat(hidden_states):
            ...
            # value attention for local feature
            value_states = self.attention.self.value(hidden_states_windows)
            value_states = self.attention.output(value_states, None)

            # >>> shifted windows >>>
            value_windows = value_states.view(-1, self.window_size, self.window_size, channels)
            shifted_windows = window_reverse(value_windows, self.window_size, height_pad, width_pad)
            # reverse cyclic shift
            if self.shift_size > 0:
                value_windows = torch.roll(shifted_windows, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
            else:
                value_windows = shifted_windows

            was_padded = pad_values[3] > 0 or pad_values[5] > 0
            if was_padded:
                value_windows = value_windows[:, :height, :width, :].contiguous()
            # <<< shifted windows <<<

            value_windows = value_windows.view(batch_size, height * width, channels)
            value_states = self.layernorm_before(value_windows)
            value_states = shortcut + self.drop_path(value_states)

            shortcut = value_states
            value_states = self.intermediate(value_states)  # fc1
            value_states = self.output(value_states)        # fc2
            value_states = shortcut + self.drop_path(self.layernorm_after(value_states))
            return value_states
        # <<< End <<<

        if not self.compute_local_feat:
            layer_outputs = residual_attn(hidden_states)
            return layer_outputs
        else:
            # >>> lyx: Add for local feature >>>
            layer_outputs = residual_attn(hidden_states)
            value_states = local_feat(hidden_states)
            return layer_outputs, value_states
            # <<< End <<<
...
class Swinv2Stage(nn.Module):
    ...
    def forward(
        ...
        hidden_states_before_downsampling = hidden_states   # (bs, h*w, c), e.g. (bs, 64*64, 128)
        if self.downsample is not None:
            height_downsampled, width_downsampled = (height + 1) // 2, (width + 1) // 2
            output_dimensions = (height, width, height_downsampled, width_downsampled)
            # (bs, h*w, c) -> (bs, h/2 * w/2, 2*c) e.g., (bs, 32*32, 256)
            hidden_states = self.downsample(hidden_states_before_downsampling, input_dimensions)
            # >>> lyx: Add for local feature >>>
            if value_states is not None:
                value_states = self.downsample(value_states, input_dimensions)
            # <<< End <<<
        else:
        ...
        # >>> lyx: Add for local feature >>>
        if value_states is not None:
            return stage_outputs, value_states # (bs, 64*64, 128)
        else:
...
class Swinv2Encoder(nn.Module):
    ...
    def forward(
        ...
        if not return_dict:
            return tuple(
                v
                for v in [hidden_states, all_hidden_states, all_self_attentions, all_reshaped_hidden_states, value_states]
                if v is not None
            )
...
class Swinv2Model(Swinv2PreTrainedModel):
    ...
    def enable_local_feat(self):
        ...
        def _enable_local_feat(module):
            """Apply `compute_local_feat=True` to all vision model components, including:
                ...
                - encoder.layers (Swinv2Stage)
                - encoder.layers.blocks (Swinv2Layer)
            """
            if hasattr(module, "compute_local_feat"):
                module.compute_local_feat = True
        self.compute_local_feat = True
        self.encoder.apply(_enable_local_feat)
    ...
    def disable_local_feat(self):
        ...
        self.compute_local_feat = False
        self.encoder.apply(_disable_local_feat)
    # <<< End <<<
    ...
    def forward(
        ...
        if self.compute_local_feat:
            ...
            value_states = self.layernorm(value_states)
        # <<< End <<<
        ...
        # >>> lyx: Add for local feature >>>
        B, HW, C = value_states.size()
        H, W = int(HW ** 0.5), int(HW ** 0.5)
        value_states = value_states.reshape(B, H, W, C).contiguous()
        encoder_outputs.value_states = value_states
        if not return_dict:
            return tuple(v for v in [sequence_output, pooled_output] + encoder_outputs[1:] if v is not None)
...
class Swinv2ForImageClassification(Swinv2PreTrainedModel):
    ...
    def enable_local_feat(self):
        ...
        def _enable_local_feat(module):
            """Apply `compute_local_feat=True` to all vision model components, including:
                - swinv2.encoder (Swinv2Encoder)
                - swinv2.encoder.layers (Swinv2Stage)
                - swinv2.encoder.layers.blocks (Swinv2Layer)
            """
            if hasattr(module, "compute_local_feat"):
                module.compute_local_feat = True
        self.compute_local_feat = True
        self.swinv2.apply(_enable_local_feat)
    ...
    def disable_local_feat(self):
        ...
        def _disable_local_feat(module):
            ...
            if hasattr(module, "compute_local_feat"):
                module.compute_local_feat = False
        self.compute_local_feat = False
        self.swinv2.apply(_disable_local_feat)
    # <<< End <<<
    ...
    def forward(
        ...
        logits_local = self.classifier(value_states)
        # <<< End <<<
